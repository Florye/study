...
<header>
	<div id="headerbar">
		<ul id="navigation">
			<li>
				<a href="${pageContext.request.contextPath}/question/questionlist/1">问答</a>
			</li>
			<li>
		...
		<ul class="log"
			<c:if test="${not empty sessionScope.user }"> style="display:none"</c:if>>
			<li><a href="javascript:void(0);" id="loglink">登录</a></li>			
		</ul>
		<ul class="log"
			<c:if test="${empty sessionScope.user }"> style="display:none"</c:if>>
