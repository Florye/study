							...
							.bind("click",
 									function() {
										if(ifAlreadyLogin()){//登陆过
											location.replace("${pageContext.request.contextPath }/question/putquestion?typeId=${requestScope.typeId}");
										}else{//没登录过
											//$("#loglink").click();//弹出登录层
											layer.msg("执行此操作需要登录",{
												icon : -1,
			...
			<script>
				$().ready(function(){
					$("#rightquestion a").bind("click",function(){
						...
						f.prop("action","${pageContext.request.contextPath}/question/questionconditionlist");
