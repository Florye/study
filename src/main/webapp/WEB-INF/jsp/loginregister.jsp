...
<script>
	...
	$(function() {
		if($("#loglink")){//假如页面存在id为loglink的元素
			$("#loglink").bind("click",function() {//登录超链接单击事件
				layer.open({
					type : 1,
					title : '登录',
		...
		$("#loginform").find("input[name='userLogin']").bind("blur",validateLoginName);//密码文本框失去焦点事件
		$("#loginform").find("input[name='userPasswd']").bind("blur",validateLoginPasswd);
		$("#logbtn").bind("click",function() {//登录按钮单击事件
...
<script>
	...
	function validateTwicePwd(){  //验证两次密码是否相同
		if(validateNewPwd()){
	...
	function validateNewPwd(){ //验证新密码数据格式
		var reg = /^[_a-zA-Z0-9]{6,10}$/;
